...
describe('ExampleComputeLiquidityValue', () => {
  ...
  beforeEach('mint some liquidity for the pool at 1:100 (100 shares minted)', async () => {
    await token0.transfer(pool.address, expandTo18Decimals(10));
    await token1.transfer(pool.address, expandTo18Decimals(1000));
    await pool.mint(wallet.address, overrides);
    expect(await pool.totalSupply()).to.eq(expandTo18Decimals(100));
  });
  ...
  describe('#getLiquidityValue', () => {
    ...
    it('correct after swap', async () => {
      await token0.approve(router.address, constants.MaxUint256, overrides);
    ...
    describe('fee on', () => {
      ...
      beforeEach('mint more liquidity to address zero', async () => {
        await token0.transfer(pool.address, expandTo18Decimals(10));
        await token1.transfer(pool.address, expandTo18Decimals(1000));
        await pool.mint(constants.AddressZero, overrides);
        expect(await pool.totalSupply()).to.eq(expandTo18Decimals(200));
      });
    ...
    describe('fee is off', () => {
      ...
      describe('after a swap', () => {
        beforeEach('swap to ~1:25', async () => {
          ...
          const [reserve0, reserve1] = await pool.slot0();
