...
import { Contract, constants, BigNumber, utils } from 'ethers';
...
describe('ExampleFlashSwap', () => {
  ...
  it('uniswapV2Call:0', async () => {
    ...
    await WETHPartner.approve(WETHExchange.address, WETHPartnerAmountV1);
    await WETHExchange.addLiquidity(BigNumber.from(1), WETHPartnerAmountV1, constants.MaxUint256, {
      ...overrides,
      value: ETHAmountV1,
    });
    ...
    const priceV2 =
      WETHPairToken0 === WETHPartner.address
        ? reservesV2[0].div(reservesV2[1])
        : reservesV2[1].div(reservesV2[0]);
  ...
  it('uniswapV2Call:1', async () => {
    ...
    await WETHExchangeV1.addLiquidity(
      BigNumber.from(1),
      WETHPartnerAmountV1,
    ...
    expect(utils.formatEther(profit)).to.eq('0.548043441089763649'); // our profit is ~.5 ETH
