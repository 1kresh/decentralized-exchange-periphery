...
library SimswapLibrary {
    ...
    bytes32 internal constant POOL_INIT_CODE_HASH =
        0x7362feddda8b9a3d60651d4926524c6c5aa42955fa7e42aba6fc90a58912feda;

    function balance(address token, address ofToken)
        internal
        view
        returns (uint256)
    {
        (bool success, bytes memory data) = token.staticcall(
            abi.encodeWithSelector(IERC20.balanceOf.selector, ofToken)
        );
    ...
    {
        if (tokenA == tokenB) revert SimswapLibrary_IDENTICAL_ADDRESSES();
        (token0, token1) = tokenA < tokenB
            ? (tokenA, tokenB)
            : (tokenB, tokenA);
        if (token0 == address(0)) revert SimswapLibrary_ZERO_ADDRESS();
    }
    ...
    function quote(
        ...
        if (amountA == 0) revert SimswapLibrary_INSUFFICIENT_AMOUNT();
    ...
    function getAmountOut(
        ...
        if (amountIn == 0) revert SimswapLibrary_INSUFFICIENT_INPUT_AMOUNT();
    ...
    function getAmountIn(
        ...
        if (amountOut == 0) revert SimswapLibrary_INSUFFICIENT_OUTPUT_AMOUNT();
        if (reserveIn == 0 || reserveOut == 0)
            revert SimswapLibrary_INSUFFICIENT_LIQUIDITY(reserveIn, reserveOut);
    ...
    function getAmountsOut(
        ...
        if (pathLengthSpecific <= 1) revert SimswapLibrary_INVALID_PATH(path);
